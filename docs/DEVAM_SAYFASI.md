...
| 4 | `interpreter-translate` Supabase fonksiyonunun deploy edilmesi | Kod yazıldı (`supabase/functions/interpreter-translate/`), yerel ortamda Supabase CLI'ya login/PAT olmadığı için deploy edilemedi. Aynı `GOOGLE_TRANSLATE_API_KEY` secret'ı kullanılıyor, yeni secret gerekmiyor. |
| 5 | RevenueCat panelinde Business entitlement + ürünleri | Kod hazır (`nura_business_monthly`/`nura_business_yearly`, entitlement id varsayılan `business`), panelde henüz tanımlanmadı. Bkz. README → "Business katmanı". |
