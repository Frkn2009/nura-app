# VOXELO — DEVAM SAYFASI (23 Ağustos 2026 — GÜNCEL)
...
- **SRS motoru**: `lib/core/algorithm/fsrs5_engine.dart` (`VoxeloBrain`, FSRS-5 tam parametreli) + `lib/data/srs/srs_machine.dart` (FSRS-inspired). `SrsCard`'da `phrase` alanı YOK.
...
- **Uygulama ikonu güncel**: `assets/icon_source.png` (yeni `VoxeloMark`'tan render edildi) + `flutter_launcher_icons` ile Android/iOS/web ikon setleri yeniden üretildi. Artık gerçek logo ile uyumlu.
...
- **Maskot**: adı **Voxelo** (eskiden "Luma" idi, tamamı değiştirildi) — `lib/ui/mascot.dart`.
- **Marka işareti**: `lib/ui/brand.dart` → `VoxeloMark` — N harfinin köşegeni ses dalgasına dönüştürüldü. `lib/ui/voxelo_logo.dart` SİLİNDİ (kullanılmayan duplicate'ti).
...
- **Görsel derinlik**: `VoxeloCard` artık düz kenarlıklı değil, açık/bal temalarda yumuşak gölgeli (koyu temada gölge yerine kenarlık — siyah zeminde gölge görünmez). Ana sayfadaki "Bugünün konuşması" kartına da renkli glow gölgesi eklendi.
...
```bash
git status --short
mkdir -p ../voxelo_backup
stamp=$(date +%Y%m%d_%H%M%S)
git diff --binary > "../voxelo_backup/before_$stamp.patch"
```
...
`git clone "<repo yolu>" C:\voxelo-build`, `android/local.properties`'e
...
Kod değiştirmeden önce git diff'i ../voxelo_backup'a yedekle (yukarıdaki
