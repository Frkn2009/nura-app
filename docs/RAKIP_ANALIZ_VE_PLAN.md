# NURA — Rakip Analizi & Eksik Özellikler (21 Ağustos 2026)

## 📊 RAKIP KARŞILAŞTIRMA TABLOSU

| Özellik | Duolingo | Babbel | Drops | Memrise | NURA |
|---|---|---|---|---|---|
| **Dil sayısı** | 40+ | 14 | 55+ | 24 | **30** ✅ |
| **Gamification** | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ ❌ |
| **Streak/Seri** | ✅ Güçlü | ✅ | ❌ | ✅ | ✅ Var ama zayıf |
| **Leaderboard** | ✅ | ❌ | ❌ | ✅ | ❌ EKSİK |
| **XP/Puan** | ✅ | ❌ | ❌ | ✅ | ❌ EKSİK |
| **Günlük hedef** | ✅ | ✅ | ✅ (5dk) | ✅ | ✅ Var |
| **Konuşma pratiği** | ⭐⭐ | ⭐⭐⭐ | ❌ | ❌ | ⭐⭐⭐⭐ ✅ AVANTAJ |
| **SRS tekrar** | Zayıf | ✅ | ❌ | ⭐⭐⭐⭐⭐ | ✅ Var |
| **Oyun modu** | ✅ Match/quiz | ❌ | ✅ Swipe | ✅ | ❌ EKSİK |
| **Çeviri** | Cümle içi | ❌ | ❌ | ❌ | Sadece sözlük ❌ |
| **Telaffuz rehberi** | ❌ | ⭐⭐ | ❌ | ❌ | ⭐⭐⭐⭐⭐ ✅ AVANTAJ |
| **Kadın ses** | Karışık | Karışık | ❌ | Gerçek video | ✅ Hep kadın |
| **Offline** | Premium | ✅ | Premium | Premium | ✅ Ücretsiz |
| **Kültürel içerik** | Az | ✅ | ❌ | ✅ Video | ✅ NIE/kira |
| **Ücretsiz katman** | Güçlü+reklam | 1 ders | 5dk/gün | Sınırlı | 1dk/gün |
| **Fiyat (yıllık)** | ~$84 | ~$84 | ~$70 | ~$60 | ~$37 ✅ UCUZ |

---

## 🏆 NURA'NIN AVANTAJLARI

1. **Konuşma odaklı** — Rakiplerin çoğu tap-tap. NURA ağız açtırıyor
2. **30 dil telaffuz rehberi** — Hiçbir rakipte yok
3. **Offline konuşma** — Cihazda TTS/STT, bulut maliyeti $0
4. **Kadın ses** — Tutarlı, güvenilir ses
5. **Ucuz** — Yıllık $37 (rakipler $60-84)
6. **NIE/kira sahneleri** — İspanya'ya giden Türk nişi
7. **30 dilde UI** — Arayüz her dilde

## ❌ NURA'NIN EKSİKLERİ (düzeltilecek)

1. **Oyun modu yok** — Kelime eşleştirme, hız yarışı lazım
2. **Leaderboard yok** — Rekabet motivasyonu
3. **XP/Puan sistemi yok** — Ödül dopamini
4. **Çeviri zayıf** — Google Translate gibi genel çeviri lazım
5. **Klipler boş** — İçerik yüklenmemiş
6. **Konuş butonu aktif değil** — Mikrofon çalışmıyor
7. **Başarım/rozet yok** — İlk 10 cümle, ilk hafta vs.
8. **Bildirim yok** — Günlük hatırlatma
9. **Animasyon/maskot yok** — Sevimlilik eksik

---

## ✅ BUGÜN EKLENMESİ GEREKENLER

### Öncelik 1: Kritik düzeltmeler
- [ ] Konuş butonu mikrofon fix
- [ ] Klipler sekmesi doldur
- [ ] Çeviri ekranı → genel çeviri (tüm diller arası)

### Öncelik 2: Oyunlaştırma
- [ ] XP puan sistemi (her sahne = XP)
- [ ] Kelime eşleştirme oyunu
- [ ] Hız yarışı (zamanlı quiz)
- [ ] Günlük meydan okuma
- [ ] Başarım rozetleri

### Öncelik 3: Sosyal
- [ ] Leaderboard (haftalık)
- [ ] Streak animasyonu (🔥)
