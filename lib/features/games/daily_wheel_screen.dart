...
class _DailyWheelScreenState extends State<DailyWheelScreen> with SingleTickerProviderStateMixin {
  ...
  void _showReward() {
    ...
    showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        title: Text('${won['icon']} Tebrikler!', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 24)),
        content: Text('${won['title']} kazandın!', style: const TextStyle(fontSize: 18)),
        actions: [
          ElevatedButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('🎡 Günlük Çark', style: TextStyle(fontWeight: FontWeight.w800))),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Text('Günlük ödülünü al!', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: NuraTokens.textSecondary)),
            const SizedBox(height: 24),
            // Ok işareti
            const Icon(Icons.arrow_drop_down, size: 48, color: NuraTokens.danger),
            // Çark
            AnimatedBuilder(
              animation: _anim,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: NuraTokens.gold),
                child: Text(
                  _spinning ? 'Dönüyor...' : _done ? 'Yarın tekrar!' : 'ÇEVİR!',
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: NuraTokens.textPrimary),
                ),
              ),
            ),
          ],
...
class _WheelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      canvas.drawArc(Rect.fromCircle(center: center, radius: radius), start, sweep, true, Paint()..color = segments[i]['color'] as Color);
      ...
      final tp = TextPainter(
        text: TextSpan(text: segments[i]['title'] as String, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 13)),
        textDirection: TextDirection.ltr,
      );
      tp.layout();
      final textPos = center + Offset.fromDirection(start + sweep / 2, radius * 0.6);
