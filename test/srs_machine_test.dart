...
void main() {
  group('NURA FSRS-inspired SRS', () {
    ...
    test('JSON round-trip preserves every field', () {
      final card = baseCard();
      final restored = SrsCard.fromJson(card.toJson());
      ...
      expect(restored.step, card.step);
      expect(restored.scheduledDays, card.scheduledDays);
      expect(restored.dueAtUtc, card.dueAtUtc);
      expect(restored.algorithmVersion, card.algorithmVersion);
      expect(restored.toJson().containsKey('phrase'), false);
    });
  });
}
